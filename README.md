# Data-Visualization
<a href="https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/MATPLOTLIB.ipynb">notebook</a>

## 1. Graph of sin.
![sin graph](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/sin_graph.png)

## 2. Graph of doted cos.
![cos doted graph](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/doted-cos-graph.png)

## 3. Overlap of two graph sin and cos on same plot.
![overlap sin and cos on same plot](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/mulitple-plot.png)

## 4. Plot with details.
![plot with details](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/graph-detail.png)

## 5. Plot of random number between from 1 to 50.
![random number plot](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/random-number-plot.png)

## 6. Points scatter.
![scatter plot](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/scatter-points.png)

## 7. Points scatter with grid.
![scatter with grid](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/graph-with-grid.png)

## 8. Cluster with grid.
![cluster with grid](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/clustered-data-plot.png)

## 9. Big(resized plot).
![resized-plot](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/resized-scatter.png)

## 10. Varible scatter.
![variable plot](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/different-sized-scatter.png)

## 11. Barchart.
![barchat-plot](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/barchat.png)

## 12. Barchart comparision.
![barchat-cmparision](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/download.png)

## 13. Horizontl graph.
![horizontal-barchart](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/horzontal-barchat.png)

## 14. Piechart.
![pychat](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/pie-chart.png)

## 15. Exploaded piechart.
![exploded-piechat](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/exploaded-piechart.png)

## 16. Donut piechart.
![donut-piechat](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/circular-hole-pi-chat.png)

## 17. Nested Donut piechart.
![nested-donut-piechart](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/nested-piechart-hole.png%202)

## 18. Two separate graph on same plot.
![separate-graph-on spame plot](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/double-plot-in-single-graph.png)

## 19. Multiple separate graph on same plote.
![multiple-graph-on same graph](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/multiple-ploat-in-single-graph.png)

## 20. 3D scatter.
![3d-scatter](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/3d-cluster.png)

## 21. 3D Cluster.
![3d-cluster](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/3d-cluster.png)

## 22. 3D Wireframe.
![3d-wireframe](https://github.com/bheemnitd/Data-Visualization-matplotlib/blob/master/3d-wireframe.png)
